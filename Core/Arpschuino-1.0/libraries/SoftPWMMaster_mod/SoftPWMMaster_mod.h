...
#include <Arduino.h>
//#define INPUT_PULLUP 0x2 //rajout christoph
...
#define SOFTPWM_DEFINE_OBJECT_WITH_BRIGHTNESS_LEVELS( CHANNEL_CNT, BRIGHTNESS_LEVELS ) \
  ...
  ISR(TIM1_COMPA_vect) {   SoftPWM.update(); }
#else
#define SOFTPWM_DEFINE_OBJECT_WITH_BRIGHTNESS_LEVELS( CHANNEL_CNT, BRIGHTNESS_LEVELS ) \
  CSoftPWM< CHANNEL_CNT, BRIGHTNESS_LEVELS > SoftPWM; \
  ISR(TIMER1_COMPA_vect) { SoftPWM.update(); }
#endif
...
*/
/*INIT originale fonctionne jusqu'� 200Hz
    TCCR1A = 0b00000000;
    TCCR1B = 0b00001001;
    OCR1A = (F_CPU - hertz * 256 / 2) / (hertz * 256);
*/
     ...
     TCCR1B = TCCR1B & 0b11111000 | 001;// 001= no prescaling - 010=scale 8 -  011=scale 64 -  100=scale 256 -  101=scale 1024 - 110= ext clock on T0 on falling edge - 111= ext clock on T0 on rising edge -
     //TCCR1B = (1 << CS10);         // start timer (ck/1 PAS DE PRESCALE)
     TCCR1A = (1 << WGM11);        // CTC mode
     OCR1A = (F_CPU/(8UL*256UL*hertz));          // We want to have at least 30Hz or else it gets choppy
     TIMSK1 = (1 << OCIE1A);       // enable timer2 output compare match interrupt
